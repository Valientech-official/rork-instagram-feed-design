...
export default function PairLookRoomScreen() {
  const router = useRouter();
  const insets = useSafeAreaInsets();
  const pairLookPosts = roomPosts.filter(post => post.roomName === 'ペアルックRoom');
  ...
  React.useEffect(() => {
    ...
    pairLookPosts.forEach(post => {
      const roomComments = getFirstTwoCommentsForRoomPost(post.id);
      initialComments[post.id] = roomComments.length > 0 ? roomComments : [
        { id: '1', username: 'ユーザー1', text: 'とても素敵なペアルックですね！', avatar: 'https://images.unsplash.com/photo-1494790108755-2616b612b786?w=100&h=100&fit=crop&crop=face' },
        { id: '2', username: 'ユーザー2', text: 'どこで購入されましたか？', avatar: 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=100&h=100&fit=crop&crop=face' },
      ];
    });
    ...
    <>
      ...
      <View style={[styles.container, { paddingTop: insets.top }]}>
        <View style={styles.headerContainer}>
          <Text style={styles.title}>ペアルックRoom</Text>
          <TouchableOpacity 
        ...
        >
          {/* Posts List */}
          {pairLookPosts.map((post, index) => (
            <View key={post.id}>
              {/* Post Display Section */}
              <View style={styles.postSection}>
                <View style={styles.postImageContainer}>
                  <Image 
                    source={{ uri: post.images?.[0] || 'https://images.unsplash.com/photo-1576566588028-4147f3842f27?q=80&w=1000' }} 
...
みなさんはどう思いますか？`}
              ...
              {index < pairLookPosts.length - 1 && <View style={styles.postSeparator} />}
            </View>
          ))}
        </ScrollView>
      </View>
